...
import { SUPPORTED_IMAGE_EXTENSIONS } from '../../utils/dist/keystone-6-core-fields-types-image-utils.esm.js';
...
function Field(_ref) {
  ...
  const accept = useMemo(() => SUPPORTED_IMAGE_EXTENSIONS.map(ext => [`.${ext}`, `image/${ext}`].join(', ')).join(', '), []);
...
function ImgView(_ref3) {
  ...
  return jsx(Stack, {
  ...
  }, errorMessage || value.kind !== 'from-server' && value.kind !== 'upload' ? jsx(Placeholder, null) : jsx(Fragment, null, value.kind === 'upload' && jsx("div", {
    css: {
      position: 'absolute',
      top: 0,
      left: 0,
  ...
  }, "Save to complete upload"), jsx("img", {
    onLoad: event => {
    ...
    src: imageSrc
  }))), value.kind === 'from-server' || value.kind === 'upload' ? onChange && jsx(Stack, {
    gap: "small",
    css: {
      height: '120px',
      justifyContent: 'flex-end'
    }
  }, errorMessage === undefined ? jsx(ImageMeta, value.kind === 'from-server' ? {
  ...
  }, "Upload Image"), value.kind === 'remove' && value.previous && jsx(Button, {
    size: "small",
    tone: "negative",
    onClick: () => {
...
function validateImage(_ref4) {
  ...
  if (!file.type.includes('image')) {
    return `Sorry, that file type isn't accepted. Please try ${SUPPORTED_IMAGE_EXTENSIONS.reduce((acc, curr, currentIndex) => {
