...
import { m as makeDataGetter } from '../../../../../dist/dataGetter-aebb736f.esm.js';
...
import { P as PageContainer, H as HEADER_HEIGHT } from '../../../../../dist/PageContainer-60ad26d2.esm.js';
...
import { C as CreateButtonLink } from '../../../../../dist/CreateButtonLink-9c1a7b9a.esm.js';
  ...
  const setNewSelectedFields = selectedFields => {
    if (isArrayEqual(selectedFields, list.initialColumns)) {
      const _router$query = router.query,
        otherQueryFields = _objectWithoutProperties(_router$query, _excluded$3);
      router.push({
  ...
  return jsx(Stack, {
    ...
    onChange: newVal => {
      ...
      if (fieldPath === noFieldOption.value) {
        const _router$query = router.query,
          restOfQuery = _objectWithoutProperties(_router$query, _excluded$1);
        router.push({
          query: restOfQuery
        });
      } else {
...
const ListPage = _ref => {
  ...
  let {
    ...
    skip: !metaQuery.data,
    variables: {
      where: _objectSpread(_objectSpread({}, filters.where), search),
      take: pageSize,
