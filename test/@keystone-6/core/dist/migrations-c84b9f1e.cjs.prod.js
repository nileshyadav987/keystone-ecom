...
var prompts = require('./prompts-82cbb393.cjs.prod.js');
...
async function pushPrismaSchemaToDatabase(dbUrl, shadowDbUrl, schema, schemaPath) {
  let shouldDropDatabase = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
  let before = Date.now();
  await ensureDatabaseExists(dbUrl, path__default["default"].dirname(schemaPath));
  let migration = await withMigrate(schemaPath, async migrate => {
    ...
    if (migration.unexecutable.length) {
      ...
      console.log('If you want to keep the data in your database, set db.useMigrations to true in your config or change the data in your database so the migration can be applied');
      if (!(await prompts.confirmPrompt(`Do you want to continue? ${chalk__default["default"].red('All data will be lost')}.`, false))) {
    ...
    if (migration.warnings.length) {
      logWarnings(migration.warnings);
      if (!(await prompts.confirmPrompt(`Do you want to continue? ${chalk__default["default"].red('Some data will be lost')}.`, false))) {
...
async function devMigrations(dbUrl, shadowDbUrl, prismaSchema, schemaPath, shouldDropDatabase) {
  await ensureDatabaseExists(dbUrl, path__default["default"].dirname(schemaPath));
...
We need to reset the ${credentials.type} database "${credentials.database}" at ${getDbLocation(credentials)}.`);
        const confirmedReset = await prompts.confirmPrompt(`Do you want to continue? ${chalk__default["default"].red('All data will be lost')}.`);
    ...
    if (evaluateDataLossResult.migrationSteps) {
      ...
      const shouldApplyMigration = migrationCanBeApplied && (await prompts.confirmPrompt('Would you like to apply this migration?', false));
...
async function getMigrationName() {
  let migrationName = await prompts.textPrompt('Name of migration');
  return slugify__default["default"](migrationName, {
    separator: '_'
...
function printMigrationId(migrationId) {
  ...
  if (words.length === 1) {
    return chalk__default["default"].cyan.bold(migrationId);
  }
  return `${words[0]}_${chalk__default["default"].cyan.bold(words.slice(1).join('_'))}`;
}
async function ensureDatabaseExists(dbUrl, schemaDir) {
  // createDatabase will return false when the database already exists
  const created = await internals.createDatabase(dbUrl, schemaDir);
  if (created) {
    const credentials = internals.uriToCredentials(dbUrl);
